...
There is a seperate vignette which covers the using `RSelenium` with Docker see `vignette("docker", package = "RSelenium")` or [here](http://ropensci.github.io/RSelenium/articles/docker.html).

#### `rsDriver`

For users who are not familiar with Docker, there is now a function `rsDriver` which will manage the binaries needed for running a Selenium Server. This provides a wrapper around the `wdman::selenium` function. For additional options and more control, see the [wdman](https://ropensci.github.io/wdman/) project. Examples using the `rsDriver` function are given in the appendix. ***Please submit any issues with running binaries to the wdman project***
...
***Note for Mac OSX:*** The default port 4444 is sometimes used by other programs such as kerberos. In our examples, we use port 4445 in respect of this and for consistency with [the Docker vignette](http://ropensci.github.io/RSelenium/articles/docker.html).
...
***Note:*** See [the Docker vignette](http://ropensci.github.io/RSelenium/articles/docker.html) for further details. The newer Docker for windows however should be accessible on the localhost.

It would have been sufficient to call `remDr <- remoteDriver(port = 4445L)`, but the options where explicitly listed to show how one may connect to an arbitrary ip/port/browser etc. More detail maybe found on [the SauceLabs vignette](http://ropensci.github.io/RSelenium/articles/saucelabs.html).
...
I will briefly touch on asynch versus sync calls here. Firstly, we need to set an appropriate asynchronous timeout (that is longer than the async operation we are likely to carryout, but it will ensure that we will eventually error out in case of an issue).
...
The `rsDriver` function is a wrapper for the `selenium` function from the [wdman](https://ropensci.github.io/wdman/) package. It allows the user to manage the binaries used to run a Selenium Server. It returns an environment containing a `client` and a `server`. 
...
For further detail and any issues you may have with the `rsDriver` function, please see the [wdman](https://ropensci.github.io/wdman/) project.
