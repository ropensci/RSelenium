...
The recommended way to run a Selenium Server is by running a Docker container. There is a seperate vignette which covers the using `RSelenium` with Docker see `vignette("docker", package = "RSelenium")`.

For users who are not familiar with Docker there is now a function `rsDriver` which will manage the binaries needed for running a Selenium Server. This provides a wrapper around the `wdman::selenium` function. For additional options and more control see the [wdman](https://github.com/ropensci/wdman) project. Examples using the `rsDriver` function are given in the appendix. ***Please submit any issues with running binaries to the wdman  project***
...
***Note:*** See the Docker vignette for further details. The newer Docker for windows however should be accessible on the localhost.
...
The `rsDriver` function is a wrapper for the `selenium` function from the [wdman](https://cran.r-project.org/package=wdman) package. It allows the user to manage the binaries used to run a Selenium Server. It returns an environment containing a "client" and a "server". 
