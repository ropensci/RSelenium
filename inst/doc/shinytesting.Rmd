...
Finally for this simple example we will look at testing the output. The two chart plots are base64 encoded images which we can check in the HTML source. The outputs should line up side by side with a maximum of 2 on a line. We can check the posistion of the outputs. The test code is in the tests folder of the `Shiny Test App` in a file named `test-output.r`.
