...
remDr$open()
remDr$navigate("http://www.google.com/ncr")
remDr$setAsyncScriptTimeout(5000)
test <- remDr$executeAsyncScript("window.setTimeout(alert('hello'), 3000); arguments[0]('DONE');", args = list())
...
```
remDr$executeAsyncScript("setTimeout(function(){ alert('Hello');},10000); arguments[arguments.length -1]('DONE');", args = list())
...
```
remDr$executeScript("setTimeout(function(){ alert('Hello');},10000); return 'DONE';", args = list())
